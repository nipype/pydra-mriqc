...
def synthstrip_wf(name="synthstrip_wf", omp_nthreads=None):
    ...
    from pydra.tasks.ants import N4BiasFieldCorrection
    from pydra.tasks.nibabel import IntensityClip, ApplyMask
    from pydra.tasks.synthstrip import SynthStrip

    wf = pydra.Workflow(name=name, input_spec=["in_files"])

    wf.add(
        IntensityClip(p_min=10, p_max=99.9, in_file=wf.lzin.in_files), name="pre_clip"
    )
    ...
    wf.add(
        N4BiasFieldCorrection(
            ...
            copy_header=True,
            input_image=wf.pre_clip.lzout.out_file,
        ),
        name="pre_n4",
    )
